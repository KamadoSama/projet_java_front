...
import { IEnseignant } from "../../../services/models/models";
...
export default function AddTeacher() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    api
      ...
      .then((res) => {
        ...
        api.getEnseignant().then((res) => setProducts(res.data));
        console.log(res);
  ...
  const [products, setProducts] = useState<IEnseignant[]>([]);
  const [productDialog, setProductDialog] = useState<boolean>(false);
  const [deleteProductDialog, setDeleteProductDialog] =
    useState<boolean>(false);
  const [deleteProductsDialog, setDeleteProductsDialog] =
    useState<boolean>(false);
  const [product, setProduct] = useState<IEnseignant>(emptyProduct);
  const [selectedProducts, setSelectedProducts] = useState<IEnseignant[]>([]);
  ...
  const dt = useRef<DataTable<IEnseignant[]>>(null);

  useEffect(() => {
    api.getEnseignant().then((res) => {
  ...
  const saveProduct = () => {
    ...
    if (product.nom.trim()) {
      ...
      if (product.idEnseignant) {
        const index = findIndexById(product.idEnseignant);
        ...
        _product.idEnseignant = createId();
  ...
  const editProduct = (product: IEnseignant) => {
    setProduct({ ...product });
    setProductDialog(true);
  };

  const confirmDeleteProduct = (product: IEnseignant) => {
    setProduct(product);
  ...
  const deleteProduct = () => {
    ...
    console.log(product.idEnseignant);
    api.deleteEnseignant(product.idEnseignant).then((res) => {
      toast.current?.show({
        severity: "success",
        summary: "Successful",
  ...
  const actionBodyTemplate = (rowData: IEnseignant) => {
    return (
      <React.Fragment>
        <Button
          icon="pi pi-pencil"
        ...
        <div className="confirmation-content">
          ...
          {product && (
            <span>
              ...
              <b>
                {product.nom} {product.idEnseignant}
              </b>
              ?
            </span>
          )}
